// pages/labSelectTime/labSelectTime.js

const app = getApp()

Page({

  data: {
    user_id: '', //用户ID
    max_reserve_days: '', //提前预约天数
    laboratory_id: '', //实验室ID
    dateList: [], //日期方块列表
    selectedDate: '', //选中的日期 (YYYY-MM-DD)
    purpose: '', //使用用途
    ExecutionTimePeriod: [], //可预约时间

    // 实验室每周开放时间列表
    labOpenTimes: [],

    timeSlots: [{
        label: '08:30-09:15',
        start: '08:30',
        end: '09:15'
      },
      {
        label: '09:25-10:10',
        start: '09:25',
        end: '10:10'
      },
      {
        label: '10:30-11:15',
        start: '10:30',
        end: '11:15'
      },
      {
        label: '11:25-12:10',
        start: '11:25',
        end: '12:10'
      },
      {
        label: '13:05-13:50',
        start: '13:05',
        end: '13:50'
      },
      {
        label: '14:00-14:45',
        start: '14:00',
        end: '14:45'
      },
      {
        label: '14:55-15:40',
        start: '14:55',
        end: '15:40'
      },
      {
        label: '15:50-16:35',
        start: '15:50',
        end: '16:35'
      },
      {
        label: '16:45-17:30',
        start: '16:45',
        end: '17:30'
      },
      {
        label: '18:30-19:15',
        start: '18:30',
        end: '19:15'
      },
      {
        label: '19:25-20:10',
        start: '19:25',
        end: '20:10'
      },
      {
        label: '20:20-21:05',
        start: '20:20',
        end: '21:05'
      }
    ],
    selectedTimes: [], //存储选中的时间段索引
    selectedTimeObjects: [], //存储选中的时间段完整信息
    labReservationArr: [], //当前实验室的所有预约
    deviceByLabReservationArr: [], //当前实验室中的设备的所有预约
    userReservationArr: [], //当前用户的所有预约（保留用于时间冲突检查）
  },

  onLoad(options) {
    const userDataStr = wx.getStorageSync('UserData')
    const userData = JSON.parse(userDataStr)

    const parseParam = (param) => {
      if (!param || param === 'undefined' || param === 'null') {
        return null
      }
      return param
    }

    this.setData({
      laboratory_id: options.laboratory_id,
      max_reserve_days: options.max_reserve_days,
      user_id: userData.user_id,

      //初始化 timeSlots，添加 selected 和 disabled 字段
      timeSlots: this.data.timeSlots.map(item => ({
        ...item,
        selected: false,
        disabled: false
      }))
    })

    //同时获取数据：实验室开放时间、实验室预约、个人预约、禁用时段
    Promise.all([
      this.getLabOpenTimes(),
      this.getDeviceByLabReservation(),
      this.getLabReservation(),
      this.getUserReservation(),
      this.selectByExecute()
    ]).then(() => {
      //生成日期列表
      this.generateDateList()
    })
  },

  onShow(){
    //同时获取数据：实验室开放时间、实验室预约、个人预约、禁用时段
    Promise.all([
      this.getLabOpenTimes(),
      this.getDeviceByLabReservation(),
      this.getLabReservation(),
      this.getUserReservation(),
      this.selectByExecute()
    ]).then(() => {
      //生成日期列表
      this.generateDateList()
    })
  },

  //获取实验室的每周开放时间设置
  getLabOpenTimes() {
    return new Promise((resolve, reject) => {
      wx.request({
        url: app.getApiUrl('/getLabOpenTimes'),
        method: 'POST',
        data: {
          laboratory_id: parseInt(this.data.laboratory_id)
        },
        header: {
          'content-type': 'application/json'
        },
        success: (res) => {
          const result = res.data
          if (result.status == 200) {
            console.log('获取的实验室开放时间：', result.data)
            this.setData({
              labOpenTimes: result.data || []
            })
            resolve()
          } else {
            reject()
          }
        },
        fail: (error) => {
          console.error('获取实验室开放时间失败:', error)
          reject(error)
        }
      })
    })
  },

  //获取当前实验室中设备的所有预约
  getDeviceByLabReservation() {
    return new Promise((resolve, reject) => {
      wx.request({
        url: app.getApiUrl('/getDeviceByLabReservation'),
        method: 'POST',
        data: {
          laboratory_id: this.data.laboratory_id
        },
        header: {
          'content-type': 'application/json'
        },
        success: (res) => {
          const result = res.data
          if (result.status == 200) {
            console.log('获取的设备预约数据：', result.data)
            this.setData({
              deviceByLabReservationArr: result.data
            })
            resolve()
          } else {
            reject()
          }
        },
        fail: (error) => {
          console.error('获取设备预约数据失败:', error)
          reject(error)
        }
      })
    })
  },

  //获取当前实验室的所有预约
  getLabReservation() {
    return new Promise((resolve, reject) => {
      wx.request({
        url: app.getApiUrl('/getLabReservation'),
        method: 'POST',
        data: {
          laboratory_id: this.data.laboratory_id
        },
        header: {
          'content-type': 'application/json'
        },
        success: (res) => {
          const result = res.data
          if (result.status == 200) {
            console.log('获取的实验室预约数据：', result.data)
            this.setData({
              labReservationArr: result.data
            })
            resolve()
          } else {
            reject()
          }
        },
        fail: (error) => {
          console.error('获取实验室预约数据失败:', error)
          reject(error)
        }
      })
    })
  },

  //获取当前用户的所有预约（保留用于可能的时间冲突检查）
  getUserReservation() {
    return new Promise((resolve, reject) => {
      wx.request({
        url: app.getApiUrl('/getLabUseReservation'),
        method: 'POST',
        data: {
          user_id: parseInt(this.data.user_id)
        },
        header: {
          'content-type': 'application/json'
        },
        success: (res) => {
          const result = res.data
          if (result.status == 200) {
            console.log('获取的用户预约数据：', result.data)
            this.setData({
              userReservationArr: result.data
            })
            resolve()
          } else {
            reject()
          }
        },
        fail: (error) => {
          console.error('获取用户预约数据失败:', error)
          reject(error)
        }
      })
    })
  },

  selectByExecute() {
    return new Promise((resolve, reject) => {
      wx.request({
        url: app.getApiUrl('/selectByExecute'),
        method: 'POST',
        header: {
          'content-type': 'application/json'
        },
        success: (res) => {
          const result = res.data
          if (result.status == 200) {
            console.log('获取的执行时段数据：', result.data)
            this.setData({
              ExecutionTimePeriod: result.data
            })
            resolve()
          } else {
            reject()
          }
        },
        fail: (error) => {
          console.error('获取执行时段数据失败:', error)
          reject(error)
        }
      })
    })
  },

  //判断某一天是否是可预约的
  isDateAvailable(date) {
    const labOpenTimes = this.data.labOpenTimes

    //如果没有设置开放时间，则不可预约
    if (!labOpenTimes || labOpenTimes.length === 0) {
      return false
    }

    //获取该日期是星期几（1-7，周一为1，周日为7）
    const weekday = date.getDay()
    const weekNum = weekday === 0 ? 7 : weekday

    //检查是否有开放时间包含这个星期
    const hasOpenTime = labOpenTimes.some(openTime => {
      const weeks = openTime.week.split(',').map(w => parseInt(w))
      return weeks.includes(weekNum)
    })

    return hasOpenTime
  },

  //标记哪些时间段是可预约的（根据每周开放时间）
  markAvailableTimeSlots() {
    const {
      labOpenTimes,
      selectedDate,
      timeSlots
    } = this.data

    if (!selectedDate || labOpenTimes.length === 0) {
      //如果没有设置开放时间，所有时间段都不可选
      const newTimeSlots = timeSlots.map(slot => ({
        ...slot,
        disabled: true,
        selected: false
      }))
      this.setData({
        timeSlots: newTimeSlots
      })
      return
    }

    //获取选中日期是星期几
    const date = new Date(selectedDate)
    const weekday = date.getDay()
    const weekNum = weekday === 0 ? 7 : weekday

    //找出该星期几对应的开放时间段
    const availableTimeRanges = []
    for (const openTime of labOpenTimes) {
      const weeks = openTime.week.split(',').map(w => parseInt(w))
      if (weeks.includes(weekNum)) {
        //处理时间，从UTC时间字符串中提取本地时间
        let startTime = openTime.start_time
        let endTime = openTime.end_time

        //如果是ISO格式字符串，直接解析并提取本地时间
        if (startTime && startTime.includes('T')) {
          //创建Date对象，自动处理时区
          const startDate = new Date(startTime)
          const endDate = new Date(endTime)

          //获取本地时间的小时和分钟
          const startHour = startDate.getHours().toString().padStart(2, '0')
          const startMin = startDate.getMinutes().toString().padStart(2, '0')
          const endHour = endDate.getHours().toString().padStart(2, '0')
          const endMin = endDate.getMinutes().toString().padStart(2, '0')

          startTime = `${startHour}:${startMin}`
          endTime = `${endHour}:${endMin}`
        } else if (startTime && startTime.includes(':')) {
          //如果是"08:30:00"格式，取前5位
          startTime = startTime.substring(0, 5)
          endTime = endTime.substring(0, 5)
        }

        availableTimeRanges.push({
          start: startTime,
          end: endTime
        })
      }
    }

    console.log('该星期可预约时间段：', availableTimeRanges)
    console.log('所有时间段：', timeSlots.map(s => ({
      label: s.label,
      start: s.start,
      end: s.end
    })))

    //标记时间段是否在开放时间内
    const newTimeSlots = timeSlots.map(slot => {
      let isAvailable = false

      //检查当前时间段是否在任一开放时间段内
      for (const range of availableTimeRanges) {
        //判断时间段是否有重叠（只要部分重叠即可预约）
        if (slot.start < range.end && slot.end > range.start) {
          isAvailable = true
          break
        }
      }

      return {
        ...slot,
        disabled: !isAvailable,
        selected: false
      }
    })

    this.setData({
      timeSlots: newTimeSlots
    }, () => {
      //然后再标记被占用的时段（在已开放的时段基础上标记占用）
      this.markOccupiedTimeSlots()
    })
  },

  //标记被占用的时间段
  markOccupiedTimeSlots() {
    const {
      labReservationArr,
      deviceByLabReservationArr,
      timeSlots,
      selectedDate,
      user_id
    } = this.data

    //合并实验室预约和设备预约
    const allReservations = [
      ...(labReservationArr || []),
      ...(deviceByLabReservationArr || [])
    ]

    console.log('当前选中日期：', selectedDate)
    console.log('合并后的预约数据：', allReservations)

    if (!allReservations.length || !selectedDate) {
      console.log('没有预约数据或未选中日期')
      return
    }

    //创建新的timeSlots数组
    const newTimeSlots = timeSlots.map(slot => {
      //如果时间段已经因为不在开放时间内而禁用，则保持不变
      if (slot.disabled) {
        return slot
      }

      //检查当前时间段在选中的日期是否被占用
      const isOccupied = allReservations.some(reservation => {
        //从预约数据中获取开始和结束时间
        const startTime = new Date(reservation.start_time)
        const endTime = new Date(reservation.end_time)

        //获取预约日期的年月日
        const resYear = startTime.getFullYear()
        const resMonth = String(startTime.getMonth() + 1).padStart(2, '0')
        const resDay = String(startTime.getDate()).padStart(2, '0')
        const resDateStr = `${resYear}-${resMonth}-${resDay}`

        //如果预约日期不等于当前选中的日期，则不冲突
        if (resDateStr !== selectedDate) return false

        //获取预约时间的小时分钟部分
        const resStartHour = String(startTime.getHours()).padStart(2, '0')
        const resStartMin = String(startTime.getMinutes()).padStart(2, '0')
        const resEndHour = String(endTime.getHours()).padStart(2, '0')
        const resEndMin = String(endTime.getMinutes()).padStart(2, '0')

        const resStart = `${resStartHour}:${resStartMin}`
        const resEnd = `${resEndHour}:${resEndMin}`

        //判断时间段是否重叠
        const occupied = (slot.start < resEnd && slot.end > resStart)
        if (occupied) {
          console.log('发现冲突（实验室或设备预约）：', slot.label)
        }
        return occupied
      })

      return {
        ...slot,
        disabled: isOccupied,
        selected: false
      }
    })

    this.setData({
      timeSlots: newTimeSlots,
      selectedTimeObjects: [],
      selectedTimes: []
    })
  },

  //生成日期方块列表（根据每周开放时间限制日期范围）
  generateDateList() {
    const today = new Date()
    const maxDays = parseInt(this.data.max_reserve_days)
    const weekdays = ['周日', '周一', '周二', '周三', '周四', '周五', '周六']
    const dateList = []

    //获取执行中的禁用时段数据
    const disabledPeriods = this.data.ExecutionTimePeriod || []

    //将禁用时段转换为日期范围列表
    const disabledDateRanges = disabledPeriods.map(period => {
      let startDateStr = period.start_time
      let endDateStr = period.end_time

      if (startDateStr.includes('T')) {
        startDateStr = startDateStr.split('T')[0]
      }
      if (endDateStr.includes('T')) {
        endDateStr = endDateStr.split('T')[0]
      }

      return {
        start: startDateStr,
        end: endDateStr
      }
    })

    for (let i = 0; i <= maxDays; i++) {
      const date = new Date(today)
      date.setDate(today.getDate() + i)

      const year = date.getFullYear()
      const month = String(date.getMonth() + 1).padStart(2, '0')
      const day = String(date.getDate()).padStart(2, '0')
      const dateStr = `${year}-${month}-${day}`

      const weekIndex = date.getDay()
      const week = weekdays[weekIndex]
      const weekNum = weekIndex === 0 ? 7 : weekIndex;

      //判断是否可预约
      let disabled = i === 0 //今天不可预约

      //检查该日期是否符合每周开放时间
      if (!disabled && this.data.labOpenTimes.length > 0) {
        if (!this.isDateAvailable(date)) {
          disabled = true
          console.log(`日期 ${dateStr} (周${weekNum}) 不在开放时间内，设为不可预约`)
        }
      }

      //检查是否在禁用时段内
      if (!disabled) {
        const isInDisabledPeriod = disabledDateRanges.some(range => {
          return dateStr >= range.start && dateStr <= range.end
        })
        if (isInDisabledPeriod) {
          disabled = true
          console.log(`日期 ${dateStr} 在禁用时段内，设为不可预约`)
        }
      }

      dateList.push({
        date: dateStr,
        display: `${month}月${day}日 ${week}`,
        year: year,
        month: month,
        day: day,
        week: week,
        weekNum: weekIndex === 0 ? 7 : weekIndex,
        disabled: disabled
      })
    }

    this.setData({
      dateList: dateList
    })
  },

  //选择日期
  onDateSelect(e) {
    const dataset = e.currentTarget.dataset
    const date = dataset.date
    const display = dataset.display

    this.setData({
      selectedDate: date,
      selectedDisplay: display,
      selectedTimeObjects: [],
      selectedTimes: []
    }, () => {
      //选中日期后，先标记可预约的时间段（根据开放时间），再标记被占用的时间段
      this.markAvailableTimeSlots()
    })

    console.log('选择的日期：', date, display)
  },

  //选择时间段（无数量限制，可多选）
  onTimeSelect(e) {
    const index = e.currentTarget.dataset.index
    const timeSlots = this.data.timeSlots

    //如果该时间段已被占用或不在开放时间内，则不能选择
    if (timeSlots[index].disabled) {
      wx.showToast({
        title: '该时间段不可预约',
        icon: 'none',
        duration: 1500
      })
      return
    }

    //切换选中/取消选中
    timeSlots[index].selected = !timeSlots[index].selected

    //更新数据
    this.setData({
      timeSlots: timeSlots
    })

    //收集选中的时间段
    const newSelectedTimeObjects = timeSlots.filter(item => item.selected).map(item => ({
      label: item.label,
      start: item.start,
      end: item.end
    }))

    this.setData({
      selectedTimeObjects: newSelectedTimeObjects
    })

    console.log('选中的时间段：', newSelectedTimeObjects)
  },

  //获取选中数量
  getSelectedCount() {
    return this.data.timeSlots.filter(item => item.selected).length
  },

  onInputPurpose(e) {
    this.setData({
      purpose: e.detail.value
    })
  },

  //确认预约
  confirmReservation() {
    const {
      selectedDate,
      selectedTimeObjects,
      purpose,
      laboratory_id,
      user_id,
    } = this.data

    if (!selectedDate) {
      wx.showToast({
        title: '请先选择预约日期',
        icon: 'none',
        duration: 1500
      })
      return
    }

    if (selectedTimeObjects.length === 0) {
      wx.showToast({
        title: '请至少选择一个时间段',
        icon: 'none',
        duration: 1500
      })
      return
    }

    if (!purpose || purpose.trim() === '') {
      wx.showToast({
        title: '请输入使用用途',
        icon: 'none',
        duration: 1500
      })
      return
    }

    //构建完整的时间段数据（加上日期）
    const timeSlots = selectedTimeObjects.map(slot => ({
      start_time: selectedDate + ' ' + slot.start + ':00',
      end_time: selectedDate + ' ' + slot.end + ':00'
    }))

    const contentLab = `预约实验室`
    const contentTime = `预约时间：${selectedDate}，共 ${selectedTimeObjects.length} 个时间段，使用用途：${purpose}`

    wx.showModal({
      title: contentLab,
      content: contentTime,
      complete: (res) => {
        if (res.cancel) {
          return
        }

        if (res.confirm) {
          wx.showLoading({
            title: '提交中...',
          })

          wx.request({
            url: app.getApiUrl('/addLabReservation'),
            method: 'POST',
            data: {
              laboratory_id: parseInt(laboratory_id),
              user_id: parseInt(user_id),
              purpose: purpose,
              time_slots: timeSlots,
            },
            header: {
              'content-type': 'application/json'
            },
            success: (res) => {
              wx.hideLoading()
              const result = res.data

              if (result.status == 200) {

                wx.reLaunch({
                  url: '/pages/index/index?reservationSuccess=true&message=预约成功'
                });

              } else if (result.status == 409) {

                //刷新页面
                this.onShow()

                wx.showToast({
                  title: '预约时间冲突，请重新选择预约时间',
                  icon: 'error',
                  duration: 1500
                })
              } else {
                wx.showToast({
                  title: result.message || '预约失败',
                  icon: 'none',
                  duration: 2000
                })
              }
            },
            fail: (error) => {
              wx.hideLoading()
              console.error('预约失败:', error)
              wx.showToast({
                title: '网络错误，请稍后重试',
                icon: 'none',
                duration: 2000
              })
            }
          })
        }
      }
    })
  },
})